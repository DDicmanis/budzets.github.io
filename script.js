...
function populateUrl() {
  //console.log("hello");
  var url = "https://kalmars.lv?";//todo pievienot 'url.com?'
  ...
  $(function () {
    $('.js-range-slider').ionRangeSlider({
      ...
      onChange: function (data) {
        ...
        //console.log(document.getElementById(data.input[0].id).value);

        document.getElementById(data.input[0].id).value = data.from;
        // var index = data.input[0].id;
        // var newIndex = index.replace('input', '');
        // twitterValues[newIndex] = data.from;
        // oldValue.forEach(function (element, index) {
        //   url = url + "&t" + index + "=" + element;
        // });
        //tweetButton.setAttribute('href', url);
        ...
        else if (inputField.value > 100) inputField.value = 100;
...
function seteditboxcolor() {
  ...
  var editboxes = d3.selectAll('.edit')
    .data(mycolors)
    .attr('bgcolor', function (d) { return colors[d]; })
}
...
function equalize() {
  ...
  if (remaining != 0) {
    ...
    if (to_eq) {
      ...
      //console.log(oldValue[d3.select(to_eq).attr('data-id')]);
